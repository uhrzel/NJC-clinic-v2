...
<body class="hold-transition sidebar-mini">
      ...
      <div class="sidebar">
        ...
        <nav class="mt-2">
          <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
            ...
            <li class="nav-item">
              <a href="generate_bill.php" class="nav-link active">
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            <!-- Hidden input fields to hold the schedule ID and patient ID -->
          ...
          <div class="modal-footer">
            ...
            <button id="generateBillButton" class="btn btn-primary">Print Receipt</button>
          </div>
        </div>
      </div>
    </div>
                ...
                <div class="card-body">
                  <table id="scheduleTable" class="table table-bordered table-striped">
                    ...
                    <tbody>
                      ...
                      // where bill_generate = 'pending'
                      // Fetch data from the schedule table
                      $sql = "SELECT * FROM schedule where bill_generate='prescription checked' order by id DESC";
                      ...
                      if (mysqli_num_rows($result) > 0) {
                        ...
                        while ($row = mysqli_fetch_assoc($result)) {
                          ...
                          echo "<td><button class='timeBtn btn-sm btn-info' >" . $row['date'] . "</button></td>";
                          echo "<td>" . $row['time'] . "</td>";
                          ...
                          echo "<a href='#' class='editSchedule btn-sm btn-success' data-id='" . $row['id'] . "' data-patient-id='" . $row['patient_id'] . "' data-toggle='modal' data-target='#generateBillModal'><i class='fas fa-file-invoice'></i> Generate Bill</a>";
                          // }else{
                          //   echo "<a href='#' class='editSchedule btn-sm btn-info' data-id='" . $row['id'] . "' data-patient-id='" . $row['patient_id'] . "' data-toggle='modal' data-target='#generateBillModal' disabled><i class='fas fa-file-invoice'></i> Generate Bill</a>";
  ...
  <script>
    ...
    $('.editSchedule').click(function() {
      // Extract data from the link's data attributes
      ...
      // Update the hidden input fields with the schedule ID and patient ID
    ...
    function printBillReceiptAndUpdateSchedule() {
      // Get the schedule ID from the hidden input field
      ...
      $.ajax({
        ...
        success: function(response) {
          ...
          // Reload the page after updating the schedule
          location.reload();
        },
    ...
    // When the "Generate Bill" button is clicked
    $('#generateBillButton').click(function() {
      // Print the bill receipt and update the schedule table
      printBillReceiptAndUpdateSchedule();
    });
  </script>
</body>
